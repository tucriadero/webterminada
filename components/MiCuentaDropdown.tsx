...
export default function MiCuentaDropdown() {
    ...
    const fetchPerfil = async () => {
      if (user) {
        const { data, error } = await supabase
          .from('profiles')
          .select('nombre, username, avatar_url, avatar_updated_at')
          .eq('id', user.id)
          .single();
  ...
  const avatarUrl =
    perfil?.avatar_url
      ? `https://cinsudwupbiqqizvqwij.supabase.co/storage/v1/object/public/avatars/${perfil.avatar_url}?v=${
          new Date(perfil.avatar_updated_at || '').getTime() || Date.now()
        }`
      : null;
      ...
      >
        {avatarUrl ? (
          <img
            src={avatarUrl}
            alt="Avatar"
            className="h-6 w-6 rounded-full object-cover border border-[#5cae97]"
          />
        ) : (
